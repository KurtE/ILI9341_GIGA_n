...
typedef ILI9341_GIGA_n displayType;
#define DISPLAY_SIZE 240
#include "config.h"     // ****** CONFIGURATION IS DONE IN HERE ******

#define RGBColor(r, g, b) ILI9341_GIGA_n::Color565(r, g, b)
...
// bugbug:: avoid new of the display
ILI9341_GIGA_n tft(eyeInfo[0].pspi, eyeInfo[0].cs, eyeInfo[0].dc,
                   eyeInfo[0].rst, eyeInfo[0].dmaStream);

#if NUM_EYES > 1
ILI9341_GIGA_n tft2(eyeInfo[1].pspi, eyeInfo[1].cs, eyeInfo[1].dc,
                    eyeInfo[1].rst, eyeInfo[1].dmaStream);
#endif
...
void setup(void) {
  ...
  for (e = 0; e < NUM_EYES; e++) {
    ...
    if (e == 0) eye[e].display = &tft;
    #if NUM_EYES > 1
    else eye[e].display = &tft2;
    #endif
  ...
  for (e = 0; e < NUM_EYES; e++) {
    Serial.print("Begin Eye: "); Serial.println(e, DEC);
    eye[e].display->begin(20000000);
  }
  Serial.println("done");
...
#ifdef ILI9341_MADCTL
    ILI9341_MADCTL, // Current TFT lib
#else
  ...
  for (e = 0; e < NUM_EYES; e++) {
    Serial.print("Use Frame buffer: "); Serial.println(e, DEC);
    if (!eye[e].display->useFrameBuffer(1)) {
      Serial.print(e, DEC);
      Serial.println(": Use Frame Buffer failed");
    } else {
      Serial.print(e, DEC);
...
void drawEye( // Renders one eye.  Inputs must be pre-clipped & valid.
  ...
  eye[e].display->setTextColor(ILI9341_RED, ILI9341_BLACK);
